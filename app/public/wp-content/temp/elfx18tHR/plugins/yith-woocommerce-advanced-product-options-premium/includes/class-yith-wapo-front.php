		...
		public function enqueue_scripts() {
			...
			if ( apply_filters( 'yith_wapo_enqueue_front_scripts', true ) ) {
				...
				$front_localize = array(
					...
					'product_quantity_selector'   => apply_filters( 'yith_wapo_product_quantity_selector', 'form.cart > .quantity input.qty' ),
				);
		...
		public function get_default_variation_price() {
			...
			if ( isset( $_POST['product_id'] ) && $_POST['product_id'] ) {
				...
				if ( $product instanceof WC_Product ) {
					...
					$values['price_html'] = $product_price;
					$values['current_price'] = wc_get_price_to_display($product);
				}
			}
			wp_send_json( $values );
		}
