...
if ( ! class_exists( 'YITH_WAPO_Cart' ) ) {
	...
	class YITH_WAPO_Cart {
		...
		public function __construct() {
			...
			add_filter( 'woocommerce_add_to_cart_validation', array( $this, 'add_to_cart_addons_validation' ), 50, 2 );
		 ...
		 * @return false|mixed
		 */
		public function add_to_cart_addons_validation( $passed, $product_id ) {
			if ( $passed ) {
				try {
					$post_data = $_POST;
					...
					if ( is_array( $addons ) && ! empty( $addons ) ) {
						foreach ( $addons as $index => $option ) {
							foreach ( $option as $addon_option => $value ) {
								...
								$values = YITH_WAPO_Premium::get_instance()->split_addon_and_option_ids( $addon_option, $value );
								...
								if ( 'product' === $info['addon_type'] ) {
									...
									$quantity   = $post_data['yith_wapo_product_qty'][ $addon_option ] ?? 1;

									if ( $product ) {
										if ( ! $product->is_in_stock() ) {
											/* translators: %s: product name */
											$message = sprintf( __( 'You cannot add &quot;%s&quot; to the cart because the product is out of stock.', 'yith-woocommerce-product-add-ons' ), $product->get_name() );
										...
										if ( ! $product->has_enough_stock( $quantity ) ) {
											...
											$message = sprintf( __( 'You cannot add that amount of &quot;%1$s&quot; to the cart because there is not enough stock (%2$s remaining).', 'yith-woocommerce-product-add-ons' ), $product->get_name(), wc_format_stock_quantity_for_display( $stock_quantity, $product ) );
										...
										if ( $product->managing_stock() ) {
											...
											if ( isset( $products_qty_in_cart[ $addon_option ] ) && ! $product->has_enough_stock( $products_qty_in_cart[ $addon_option ] + $quantity ) ) {
												...
												$message = sprintf(
													...
													wc_get_cart_url(),
													__( 'View cart', 'yith-woocommerce-product-add-ons' ),
													/* translators: 1: quantity in stock 2: current quantity */
													sprintf( __( 'You cannot add that amount to the cart &mdash; we have %1$s in stock and you already have %2$s in your cart.', 'yith-woocommerce-product-add-ons' ), wc_format_stock_quantity_for_display( $stock_quantity, $product ), wc_format_stock_quantity_for_display( $stock_quantity_in_cart, $product ) )
												);
		...
		public function add_cart_item_data( $cart_item_data, $product_id, $post_data = null, $sold_individually = false ) {
			...
			if ( isset( $cart_item_data['bundled_by'] ) || ( isset( $post_data['yith_wapo_product_id'] ) && intval( $product_id ) !== intval( $post_data['yith_wapo_product_id'] ) ) ) {
				return $cart_item_data;
			}
			$data = array();

			if ( isset( $post_data['yith_wapo'] ) && is_array( $post_data['yith_wapo'] ) ) {
				$cart_item_data['yith_wapo_product_img'] = ! empty( $post_data['yith_wapo_product_img'] ) ? attachment_url_to_postid( $post_data['yith_wapo_product_img'] ) : 0;
				...
				foreach ( $cart_item['yith_wapo_options'] as $index => $option ) {
					foreach ( $option as $key => $value ) {
						if ( $key && '' !== $value ) {
							...
							$addon_data_name = $this->get_addon_data_name( $addon_id, $option_id );
							...
							if ( 'yes' === get_option( 'yith_wapo_show_options_in_cart', 'yes' ) ) {
								if ( ! $is_empty_select ) {
									...
									$cart_data_array[ $addon_data_name ] .= '<div>' . $addon_value . ( '' !== $option_price && floatval( 0 ) !== floatval( $option_price ) ? ' (' . $sign . wc_price( $option_price ) . ')' : '' ) . '</div>';
								}
							}
							...
							if ( $grouped_in_cart ) {
								if ( ! $is_empty_select ) {
									$cart_data[] = array(
										'name'    => $addon_label,
										'display' => empty( $option_price ) ? $addon_value : '<div>' . $addon_value . ( '' !== $option_price && floatval( 0 ) !== floatval( $option_price ) ? ' (' . $sign . wc_price( $option_price ) . ')' : '' ) . '</div>',
									);
								}
							}
						}
				...
				foreach ( $cart_item['yith_wapo_options'] as $index => $option ) {
					foreach ( $option as $key => $value ) {
						if ( $key && '' !== $value ) {
							...
							if ( '' === $addon_name ) {
								$addon_name = apply_filters( 'yith_wapo_order_item_meta_name_default', __( 'Option', 'yith-woocommerce-product-add-ons' ), $index, $item_id, $cart_item );
							}
							...
							$display_value = $addon_value . ( '' !== $option_price && floatval( 0 ) !== floatval( $option_price ) ? ' (' . $sign . wc_price( $option_price ) . ')' : '' );

							wc_add_order_item_meta( $item_id, $addon_name, stripslashes( $display_value ) );
						}
					}
				}
		...
		public function calculate_addon_prices_on_cart( $addon_id, $option_id, $key, $value, $cart_item, $product_price, $calculate_taxes = true ) {
			...
			$addon_price      = ! empty( $addon_price ) ? $addon_price * $option_product_qty : 0;
			$addon_price_sale = ! empty( $addon_price_sale ) ? $addon_price_sale * $option_product_qty : 0;
		...
		public function get_addon_value_on_cart( $addon_id, $option_id, $key, $original_value, $cart_item, $grouped_in_cart = false ) {
			...
			if ( 'product' === $addon_type ) {
				...
				$option_product      = wc_get_product( $option_product_id );
				if ( $option_product instanceof WC_Product ) {
					$value = $option_product_qty . ' x ' . $option_product->get_name();
				}
			} elseif ( 'color' === $addon_type ) {
				if ( ! empty( $color ) ) {
					if ( $grouped_in_cart ) {
						$value = $color;
					} else {
						$show_option_color = apply_filters( 'yith_wapo_show_option_color', true );
						if ( $show_option_color ) {
			...
			} elseif ( in_array( $addon_type, array( 'text', 'textarea', 'number', 'color', 'date', 'colorpicker' ) ) ) {
				if ( ! $grouped_in_cart ) {
					$label = ! empty( $label ) ? $label . ': ' : '';
				...
				$file_url  = explode( '/', $original_value );
				$file_name = apply_filters( 'yith_wapo_show_attached_file_name', true ) ? end( $file_url ) : __( 'Attached file', 'yith-woocommerce-product-add-ons' );
				$value      = '<a href="' . $original_value . '" target="_blank">' . $file_name . '</a>';
			} elseif ( in_array( $addon_type, array( 'select', 'radio', 'label', 'checkbox' ) ) ) {
				$value = $label;
			}
		...
		public function get_addon_data_name( $addon_id, $option_id, $grouped_in_cart = false ) {
			...
			if ( ! $grouped_in_cart || empty( $addon_label ) ) {
				$addon_label = $addon_title;
			}
