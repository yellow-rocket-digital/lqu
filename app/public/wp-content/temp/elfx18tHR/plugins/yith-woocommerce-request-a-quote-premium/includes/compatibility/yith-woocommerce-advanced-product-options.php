	...
	class YWRAQ_Avanced_Product_Options {
		...
		public function request_quote_view( $item_data, $raq, $_product, $show_price = true ) {
			...
			if ( isset( $raq['yith_wapo_options'] ) ) {
				// > 3.7.0
				if ( defined( 'YITH_WAPO_VERSION' ) && YITH_WAPO_VERSION >= '3.7.0' ) {
					...
					foreach ( $raq['yith_wapo_options'] as $index => $option ) {
						...
						foreach ( $option as $key => $value ) {
							if ( $key && '' !== $value ) {
								...
								$addon_value     = $addon_cart_instance->get_addon_value_on_cart( $addon_id, $option_id, $key, $value, $cart_item );
								...
								$item_data[] = array(
									'key'   => $addon_data_name . apply_filters( 'yith_wapo_cart_item_addon_price', $price_html ),
				...
				// 2.x
				else if ( defined( 'YITH_WAPO_VERSION' ) && YITH_WAPO_VERSION <= '3.7.0' && function_exists( 'yith_wapo_get_option_info' ) ) {
		...
		public function get_addon_image( $image, $raq, $get_src = false ) {
			if ( isset( $raq['yith_wapo_product_img'] ) ) {
				...
				$image    = $get_src ? current( wp_get_attachment_image_src( $image_id, 'medium' ) ) : wp_get_attachment_image( $image_id, 'medium' );
			}
