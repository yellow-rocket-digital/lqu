...
(function ($) {
	...
	window.GFPPCP = function (args) {
		...
		self.init = function () {
			...
			this.stateManager = new _state_manager__WEBPACK_IMPORTED_MODULE_6__["default"](initialState, self.handleStateChange.bind(this));
   ...
   */
		self.showHidePayPalField = function (display) {
			...
			if (prevDisplay != display) {
				...
				self.stateManager.setDirty('fieldVisible');
			}
		};
		/**
   ...
   */
		self.handleProductTotalFilter = function (total, formId) {
			...
			self.stateManager.set('total', total);
			return total;
		};
   ...
   */
		self.handleFrontendFeedsEvaluated = function (feeds, formId) {
			...
			self.stateManager.set('activeFeed', activeFeed);
			...
			if (paymentMethodChanged) {
				...
				self.stateManager.set('paymentMethod', paymentMethods[0]);
			}
		};
   ...
   */
		self.handlePostConditionalLogic = function (formId, action, targetId, defaultValues, isInit) {
			...
			if (formId != self.formId) {
				...
				self.stateManager.set('fieldVisible', action == 'show');
			} else if (targetId == self.submitButtonId) {
				self.stateManager.set('submitVisible', action == 'show');
			}
		};
   ...
   */
		self.bindStateEvents = function () {
			...
			$(self.form).on('select change', function () {
				self.stateManager.set('currency', self.getCurrentCurrencyCode());
			});

			// Binding to payment method change ( i.e. Credit Card or PayPal Checkout ).
			self.paymentMethodField.on('change', function () {
				self.stateManager.set('paymentMethod', self.paymentMethodField.val());
				self.resetCardErrors();
			});
		};
   ...
   */
		self.handleFormSubmission = function (event) {
			...
			if (self.isGoPrevPage()) {
				self.removeIdFields();
			} else if (self.stateManager.get('paymentMethod') == 'Credit Card' && self.stateManager.get('activeFeed').intent == 'subscription') {
			...
			var continueDefaultFormSubmission = !self.stateManager.get('activeFeed') || self.stateManager.get('paymentMethod') !== 'Credit Card' || $('#gform_save_' + self.formId).val() === '1' || $(self.form).data('gfppcpsubmitting') || self.paymentAmount <= 0 || !self.isCreditCardOnPage() || self.isGoPrevPage() || typeof gformIsRecaptchaPending !== 'undefined' && gformIsRecaptchaPending($(self.form));
   ...
   */
    ...
    */
		...
		self.getOrderData = function () {
			...
			$.ajax({
				...
				data: {
					...
					form_id: self.formId,
					feed_id: self.stateManager.get('activeFeed').feedId,
		...
		self.updateOrderDetails = function () {
			var activeFeed = self.stateManager.get('activeFeed');
				...
				while (1) {
					switch (_context.prev = _context.next) {
						...
						case 3:
							...
							self.countryCode = self.getMergeTagValue(self.stateManager.get('activeFeed').address_country);
