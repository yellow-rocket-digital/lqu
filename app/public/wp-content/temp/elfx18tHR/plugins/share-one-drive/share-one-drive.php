...
// SYSTEM SETTINGS
define('SHAREONEDRIVE_VERSION', '2.1.1');
...
class Core
{
    ...
    {
        if ((version_compare(PHP_VERSION, '7.4') < 0) || (!function_exists('curl_init'))) {
            return false;
        }
...
%filelist%
  ...
  <td style="font-weight: bold;">%file_size%</td>
        ...
        if (!isset($this->settings['userfolder_backend_auto_root']) || '' === $this->settings['userfolder_backend_auto_root']) {
...
<p>Your account isn't (yet) configured to access this content. Please contact the administrator of the site if you would like to have access. The administrator can link your account to the right content.</p>";
        ...
        if (SHAREONEDRIVE_VERSION !== $version) {
            delete_transient('shareonedrive_activation_validated');
            delete_site_transient('shareonedrive_activation_validated');
