...
<SyncStatusRequest xmlns="http://www.intuit.com/sb/cdm/v2" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.intuit.com/sb/cdm/xmlrequest RestDataFilter.xsd">
	...
	*/
	...
	protected function _upload($Context, $realmID, $resource, $Object,$imgBits,$fileName,$mimeType){
		// Generate the XML
		$xml = $Object->asXML(0, null, null, null, QuickBooks_IPP_IDS::VERSION_3);
        ...
        $dataMultipart .= "Content-Type: " . $ct_xml . '; charset=UTF-8' . $newline;
		...
		#$dataMultipart .= base64_encode($imgBits) . $newline;
		#$dataMultipart .= $imgBits . $newline;
        $dataMultipart .= "--" . $boundaryString . "--" . $newline . $newline; // finish with two eol's!!
		
		$uprd = array();
		$uprd['boundaryString'] = $boundaryString;
		
		// v3 only
		$IPP = $Context->IPP();
		
		// Send the data to IPP
		$return = $IPP->IDS($Context, $realmID, $resource, QuickBooks_IPP_IDS::OPTYPE_UPLOAD, $dataMultipart,null,$uprd);		
