 ...
 */
abstract class SODOneDrive_Logger_Abstract
{
  /**
   * Default log format
   */
  ...
  /**
   * @param SODOneDrive_Client $client  The current OneDrive client
   */
  public function __construct(SODOneDrive_Client $client)
  {
    $this->setLevel(
        $client->getClassConfig('SODOneDrive_Logger_Abstract', 'level')
    );

    $format = $client->getClassConfig('SODOneDrive_Logger_Abstract', 'log_format');
    $this->logFormat = $format ? $format : self::DEFAULT_LOG_FORMAT;

    $format = $client->getClassConfig('SODOneDrive_Logger_Abstract', 'date_format');
   ...
   * @throws SODOneDrive_Logger_Exception If $level is invalid
   */
  protected function normalizeLevel($level)
  {
    ...
    throw new SODOneDrive_Logger_Exception(
