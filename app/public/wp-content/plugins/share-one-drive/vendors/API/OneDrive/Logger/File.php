 ...
 */
class SODOneDrive_Logger_File extends SODOneDrive_Logger_Abstract
{
  /**
  ...
  public function __construct(SODOneDrive_Client $client)
  {
    ...
    $file = $client->getClassConfig('SODOneDrive_Logger_File', 'file');
    ...
    $mode = $client->getClassConfig('SODOneDrive_Logger_File', 'mode');
    if (!$mode) {
      $this->mode = $mode;
    }

    $this->lock = (bool) $client->getClassConfig('SODOneDrive_Logger_File', 'lock');
    $this->file = $file;
  }
  ...
  protected function write($message)
  {
    if (is_string($this->file)) {
      ...
      throw new SODOneDrive_Logger_Exception('File pointer is no longer available');
    }
