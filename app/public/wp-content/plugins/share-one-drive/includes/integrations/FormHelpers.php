...
class FormHelpers
{
    ...
    {
        add_filter('shareonedrive_render_formfield_data', [$this, 'render_form_value'], 10, 3);
    }
    ...
    public function render_form_value($data, $ashtml, $formclass)
    {
        ...
        $folder_name = (in_array(dirname($first_entry->path), ['\\', '/']) ? 'OneDrive' : dirname($first_entry->path));
    ...
    <tr style="<?php echo ($current % 2) ? 'background: #fafafa;' : ''; ?> height: 26px;">
        <td style="width:20px;padding-right:10px;padding-left:5px;border:none;">
            <img alt="" height="16" src="<?php echo \TheLion\ShareoneDrive\Helpers::get_default_icon($file->type, false); ?>" style="border:0;display:block;outline:none;text-decoration:none;height:auto;width:16px;max-width:16px;" width="16">
        </td>
