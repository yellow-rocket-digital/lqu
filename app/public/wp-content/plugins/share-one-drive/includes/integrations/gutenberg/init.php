...
class Gutenberg
{
    ...
    {// phpcs:ignore
        wp_register_script(
            'wpcp-shareonedrive-block-js',
            plugins_url('/dist/blocks.build.js', __FILE__),
            ['wp-blocks', 'wp-i18n', 'wp-element', 'wp-editor'],
            defined('SHAREONEDRIVE_VERSION') ? SHAREONEDRIVE_VERSION : filemtime(plugin_dir_path(__DIR__).'dist/blocks.build.js'),
            true
        );

        // Register block editor styles for backend.
        wp_register_style(
            'wpcp-shareonedrive-block-editor-css',
            plugins_url('dist/blocks.editor.build.css', __FILE__),
            ['wp-edit-blocks'],
            defined('SHAREONEDRIVE_VERSION') ? SHAREONEDRIVE_VERSION : filemtime(plugin_dir_path(__DIR__).'dist/blocks.editor.build.css')
        );

        $i18n = [
            'title' => 'OneDrive',
            'description' => sprintf(\esc_html__('Insert your %s content', 'wpcloudplugins'), 'OneDrive'),
            'form_keywords' => [
                'cloud',
                'onedrive',
                'drive',
        ...
        // WP Localized globals. Use dynamic PHP stuff in JavaScript via `wpcpShareoneDriveGlobal` object.
        wp_localize_script(
            'wpcp-shareonedrive-block-js',
            'wpcpShareoneDriveGlobal',
            [
                'pluginDirPath' => plugin_dir_path(__DIR__),
                'pluginDirUrl' => plugin_dir_url(__DIR__),
                'ajaxUrl' => admin_url('admin-ajax.php'),
                'i18n' => $i18n,
                'wpnonce' => \wp_create_nonce('wpcp-shareonedrive-block'),
                'editable' => \TheLion\ShareoneDrive\Helpers::check_user_role(\TheLion\ShareoneDrive\Core::get_setting('permissions_add_shortcodes')) ? 1  : 0,
            ]
        );
        ...
        register_block_type(
            'wpcp/shareonedrive-block',
            [
                'attributes' => $attributes,
                // Enqueue blocks.build.js in the editor only.
                'editor_script' => 'wpcp-shareonedrive-block-js',
                // Enqueue blocks.editor.build.css in the editor only.
                'editor_style' => 'wpcp-shareonedrive-block-editor-css',
