...
$datepicker_args = apply_filters( 'yith_wapo_datepicker_options', array(
    'dateFormat' => "'" . esc_attr( $date_format_js ) . "'",
) );

if ( $start_year > 0 ) {
    $datepicker_args['minDate'] = "new Date ('" . esc_attr( $start_year ) . "-01-01')";
}
if ( $end_year > 0 ) {
    $datepicker_args['maxDate'] = "new Date ('" . esc_attr( $end_year ) . "-12-31')";
}
...
<div id="yith-wapo-option-<?php echo esc_attr( $addon->id ); ?>-<?php echo esc_attr( $x ); ?>" class="yith-wapo-option">
	...
	<?php if ( $addon->get_option( 'show_image', $x, 'no', false ) && $addon->get_option( 'image', $x ) !== '' && 'yes' !== $setting_hide_images ) : ?>
		<?php
		...
		jQuery( function() {
			...
			initializeDatePickers = function () {
				jQuery( '#yith-wapo-<?php echo esc_attr( $addon->id ); ?>-<?php echo esc_attr( $x ); ?>' ).datepicker({
                    <?php
                        foreach ( $datepicker_args as $arg => $arg_value ){
                            echo wp_kses_post( $arg . ':' . $arg_value . ',' );
                        }
                    ?>
			...
			<?php
			if ( $datepicker_timeout ) {
				?>
				setTimeout( initializeDatePickers, 3000 );
				<?php
			} else {
				?>
